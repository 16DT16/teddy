...
const BOOTSTRAP_CACHE_KEY = "ambo-menafesha-bootstrap-v1";
...
export default function CustomerOrderPage() {
  ...
  function applyBootstrapData(data: BootstrapData) {
    ...
    setGojoId((current) =>
      nextGojos.some((gojo) => gojo.id === current)
        ? current
        : nextGojos[0]?.id || ""
    );
    ...
    return { nextGojos, nextProducts };
  }

  async function loadBootstrap(showFullLoader = true) {
    try {
      if (showFullLoader) setBootLoading(true);

      const data = (await readJsonSafe("/api/bootstrap")) as BootstrapData;
      const { nextGojos, nextProducts } = applyBootstrapData(data);
      ...
      if (!products.length || !gojos.length) {
        setNotice({
          type: "error",
          text:
            error?.message ||
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (cached) {
        ...
        const { nextGojos, nextProducts } = applyBootstrapData(parsed);
        restoredFromCache = Boolean(nextGojos.length && nextProducts.length);
    ...
    loadBootstrap(!restoredFromCache);
  }, []);
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_#edf8ef_0,_#f5f7f2_38%,_#eef2ea_100%)] px-3 py-3 sm:px-6 sm:py-6 lg:px-8">
      <section className="mx-auto max-w-7xl">
        <header className="mb-5 overflow-hidden rounded-[1.75rem] border border-white/10 bg-[linear-gradient(135deg,#052e1a_0%,#064e2b_58%,#0a6b3d_100%)] shadow-[0_24px_70px_rgba(5,46,26,.22)]">
        ...
        <div className="grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
          ...
          <aside className="overflow-hidden rounded-[2rem] border border-[#064e2b]/10 bg-white shadow-[0_18px_55px_rgba(5,46,26,.08)] lg:sticky lg:top-5 lg:max-h-[calc(100vh-2.5rem)]">
            <div className="border-b border-[#064e2b]/8 bg-white/95 p-5 backdrop-blur">
            <div className="mb-4 flex items-center justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[.22em] text-[#087443]">
                  የዛሬ ሜኑ
            ...
            <div className="max-h-[calc(100vh-130px)] overflow-y-auto p-5 [scrollbar-color:#9bc9aa_transparent] [scrollbar-width:thin]">
            {bootLoading && (
              <div className="rounded-[1.5rem] border border-[#064e2b]/8 bg-[#f7fbf2] p-5 text-center">
                <Loader2
                  size={24}
                  className="mx-auto animate-spin text-[#087443]"
                />
            ...
            {!bootLoading && groupedProducts.length > 0 && (
              <div className="mb-4 flex items-center justify-between rounded-2xl border border-[#087443]/10 bg-[#f3fbf5] px-4 py-3">
                <div>
                  <p className="text-xs font-black uppercase tracking-[.16em] text-[#087443]">የሚገኙ እቃዎች</p>
                  <p className="mt-0.5 text-sm font-bold text-[#052e1a]">{products.length} የሜኑ እቃዎች</p>
                </div>
                <button
                  type="button"
                  onClick={() => loadBootstrap(false)}
                  className="grid h-10 w-10 place-items-center rounded-xl bg-white text-[#087443] shadow-sm transition hover:-translate-y-0.5 hover:bg-[#ecfff4]"
                  aria-label="ሜኑን አድስ"
                >
                  <RefreshCw size={17} />
                </button>
                    ...
                    {group.items.map((product) => {
                      ...
                      return (
                        <button
                          type="button"
                          key={product.id}
                          onPointerDown={() => setProductId(product.id)}
                          onClick={() => setProductId(product.id)}
                          className={`group w-full rounded-2xl border px-4 py-3.5 text-left transition duration-150 active:scale-[.99] ${
                            isSelected
                              ? "border-[#087443] bg-[linear-gradient(135deg,#ecfff4,#f7fff9)] shadow-[0_8px_24px_rgba(8,116,67,.10)]"
                              : "border-[#064e2b]/8 bg-[#fbfdf8] hover:-translate-y-0.5 hover:border-[#087443]/25 hover:bg-white hover:shadow-sm"
                          }`}
                        >
                          <div className="flex items-center justify-between gap-4">
                            <div>
