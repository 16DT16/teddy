...
export default function AdminPage() {
  ...
  const lastSuccessfulSummaryRef = useRef<Summary | null>(null);
  ...
  async function load(
    ...
    try {
      ...
      const nextSummary: Summary = {
        byGojo: Array.isArray(data.byGojo)
          ? data.byGojo
          : [],
      ...
      lastSuccessfulSummaryRef.current =
        nextSummary;
      ...
      setNotice({
        type: "info",
        text: online
  ? "ግንኙነቱ ዘግይቷል። የመጨረሻው የተሳካ ሪፖርት እየታየ ነው። እንደገና ለመሞከር Refresh Dashboard ይጫኑ።"
  : "የኢንተርኔት ግንኙነት የለም። የመጨረሻው ሪፖርት እየታየ ነው።",
      });

      if (manual) {
        console.warn(
          "Manual admin refresh failed:",
          error,
        );
      }
      ...
      setNotice({
        ...
        text:
          error instanceof Error
            ? error.message
            : "ምርቶቹን መጫን አልተቻለም። ያለው መረጃ እንዳለ ተጠብቋል።",
      });
    } finally {
  ...
  async function saveStaff(
    ...
    if (
      ...
      setNotice({
        type: "error",
        text:
          "A password is required for a new staff account.",
      });
      return;
    }
   ...
   * Load again only when the selected sales range changes.
   */
  void load(
    false,
    salesRange,
  ...
  function handleOnline() {
     ...
     * Just show that the connection is available again.
     */
    setConnectionState("online");
    ...
    setNotice((current) => {
      if (
        ...
        return {
          type: "success",
          text:
            "የኢንተርኔት ግንኙነቱ ተመልሷል። የቅርብ መረጃ ለማየት Refresh Dashboard ይጫኑ።",
        };
      }
  ...
  function handleOffline() {
    ...
    setNotice({
      type: "info",
      text:
        "የኢንተርኔት ግንኙነት ተቋርጧል። የመጨረሻው የተጫነ ሪፖርት እየታየ ነው።",
    });
  }
  ...
  // load is intentionally excluded.
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm font-semibold text-amber-800/75">
                  {summary
  ? "የመጨረሻው የተሳካ ሪፖርት እየታየ ነው። አዲስ መረጃ ለመጫን Refresh Dashboard ይጫኑ።"
  : "ሪፖርቱን ለመጫን እንደገና ይሞክሩ።"}
                </p>
